...
public class gyroSampler implements Runnable {
	...
	public gyroSampler(GyroITG3200 gyro) { // Maybe change this to the other
											// mentor's code on gitHub
	...
	public void run() {
		...
		while (isLoopRunning) {
			...
			// SmartDashboard.putNumber("Gyro Raw Value form Thread",
			// raw_angle);
	...
	public void stopGyro() {
		isLoopRunning = false; // Thread should exit because while loop is
								// finished
	}
	...
	public int pidAngle() {
		// Converts angle from 0-360 to the range of -179 - 180 where 0 is the
		// center
		int pidAngle = 0;
		if (gyro_angle == 0) {
			pidAngle = 0;
